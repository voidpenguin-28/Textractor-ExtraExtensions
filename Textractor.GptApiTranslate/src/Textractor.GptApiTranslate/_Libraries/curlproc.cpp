...
namespace curlproc {
#define BUFSIZE 4096
	string buildCurlCommand(const string& url, const string& method = "GET", const string& postData = "", const vector<string>& headers = vector<string>(), int connectTimeoutSecs = 10, const string& userAgent = "", const string& customCurlPath = "");
	string formatPath(string path);
    string formatPostData(const string& postData);
    ...
    string httpGet(const string& url, const vector<string>& headers, int connectTimeoutSecs, const string& userAgent, const string& customCurlPath) {
		string command = buildCurlCommand(url, "GET", "", headers, connectTimeoutSecs, userAgent, customCurlPath);
		string output = runProcess(command);
		return output;
	}

    string httpPost(const string& url, const string& body, const vector<string>& headers, int connectTimeoutSecs, const string& userAgent, const string& customCurlPath) {
		string command = buildCurlCommand(url, "POST", body, headers, connectTimeoutSecs, userAgent, customCurlPath);
		string output = runProcess(command);
		return output;
	}

    string buildCurlCommand(const string& url, const string& method, const string& postData, const vector<string>& headers, int connectTimeoutSecs, const string& userAgent, const string& customCurlPath) {
		string command = formatPath(customCurlPath);
		command += "curl -L -s";

		if (method.length() > 0) command += " -X " + method;
        command += " -m " + to_string(connectTimeoutSecs);
		...
		if (postData.length() > 0) command += " -d \"" + formatPostData(postData) + "\"";
    ...
    string formatPostData(const string& postData) {
        string formattedData = replace(postData, "\\\"", "\\\\\"");
