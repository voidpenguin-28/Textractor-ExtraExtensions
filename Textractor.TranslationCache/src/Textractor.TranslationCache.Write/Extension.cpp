...
const string _backupModuleName = "TranslationCache.Write";
unique_ptr<ExtensionDepsContainer> _deps = nullptr;


inline void allocateResources(const HMODULE& hModule) {
	_deps = make_unique<DefaultExtensionDepsContainer>(hModule, false);
...
inline void deallocateResources() {
	if (_deps != nullptr) clearCacheIfUnload();
	_deps = nullptr;
}
...
bool ProcessSentence(wstring& sentence, SentenceInfo sentenceInfo)
{
	try {
		...
		sentence = _deps->getCacheManager().writeCacheOrLoadTemp(sentence, sentInfoWrapper, config);
		return true;
	}
